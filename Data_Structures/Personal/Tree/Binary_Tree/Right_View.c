...
void right_view (Tree* root, int level, int* max)
{
	...
	right_view (root->right, (level + 1), max);		//Firstly, Traverse the Right Sub-Tree of the Root Node.
	right_view (root->left, (level + 1), max);		//Then, Traverse the Left Sub-Tree of the Root Node.
}
...
int main()
{
	...
	/* Right View of the Tree: The set of Rightmost Nodes for every Level */
	printf ("The Right View of the given Tree:\n");
	int max = 0;										//To store the Levels which are visited and the First Node visible from Right side is printed.
	right_view (root, 1, &max);							//Assume the Levels of the Tree start from '1'.
	printf ("\n");
	return 0;
}
